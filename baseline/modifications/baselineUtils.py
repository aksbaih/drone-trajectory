...
def create_dataset(dataset_folder,dataset_name,val_size,gt,horizon,delim="\t",train=True,eval=False,verbose=False):
        ...
        for i_dt, dt in enumerate(datasets_list):
            ...
            # inp and out are now slices of size gt+horizon containing abs and rel positions.. 1 frame steps
...
def get_strided_data_clust(dt, gt_size, horizon, step):
    ...
    #inp_relative_pos= inp_te_np-inp_te_np[:,:1,:]
    inp_speed = np.concatenate((np.zeros((inp_te_np.shape[0],1,inp_te_np.shape[-1])),inp_te_np[:,1:,:] - inp_te_np[:, :-1, :]),1)

    # inp_speed is relative deltas
    ...
    # inp_norm is concat of absolute locations and deltas

    inp_mean=np.zeros(inp_norm.shape[-1])
    inp_std=np.ones(inp_norm.shape[-1])
