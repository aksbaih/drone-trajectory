"""
This code is adopted from the github.com/uzh-rpg/uzh_fpv_open repo with code added for download convenience by
Akram Sbaih (akram at stanford dot edu).
"""
...
if __name__=="__main__":
    ...
    if not os.path.exists('data/t'): os.mkdir('data/t')
    if not os.path.exists('data/xyz'): os.mkdir('data/xyz')
    for unzip in tqdm(unzips):
        identifier = unzip.split('/')[-1]
        t, xyz = parseTextFile(unzip+'/leica.txt')
        np.save('data/t/'+identifier, t)
        np.save('data/xyz/'+identifier, xyz)
    print("Done!")
